...
def _find_window_handle_by_exe_name(exe_name_startswith, max_attempts=120, delay_s=0.5):
    ...
    if _enum_windows_callback_found_hwnds:
        ...
        print(f"未找到可执行文件名为'{exe_name_startswith}'开头的任何窗口。")
...
class GameLauncher(QWidget):
    ...
    def __init__(self):
        ...
        self.setGeometry(100, 100, 1440, 810)  # 设置窗口的初始位置和大小
        ...
        self.settings_file_path = os.path.join(self.current_script_dir, SETTINGS_FILE)  # 定义设置文件的完整路径
        ...
        self.max_game_check_attempts = 60  # 最多检查 60 秒 (每秒检查一次)
        ...
        self.transition_speed_ms = 300  # 300毫秒过渡速度
        ...
        print(f"GameLauncher: 初始 core_program_path: {self.core_program_path}") # Debugging line

        # 启动序列：首先启动后台脚本，然后加载数据，最后初始化并更新 UI
        print("GameLauncher: 启动后台脚本。")
        self.start_background_scripts()
        print("GameLauncher: 加载抓取数据 JSON。")
        ...
        # 单次定时器，确保在 UI 完全初始化后更新游戏路径 UI
        print("GameLauncher: 计划初始 UI 更新游戏路径。")
        ...
        if os.path.exists(self.settings_file_path):
            try:
                with open(self.settings_file_path, 'r', encoding='utf-8') as f:
                    ...
                    if path and os.path.isfile(path):  # 检查路径是否存在且为文件
                        print(f"load_core_program_path_setting: 从 {SETTINGS_FILE} 加载核心程序路径：{path}")
                        return path
                    elif path:
                        print(
                            f"load_core_program_path_setting: 警告：{SETTINGS_FILE} 中的核心程序路径 '{path}' 无效或文件不存在。")
            except json.JSONDecodeError as e:
                print(
                    f"load_core_program_path_setting: 错误：解析 {SETTINGS_FILE} 时核心程序部分发生 JSON 解码错误：{e}。")
            except Exception as e:
                print(f"load_core_program_path_setting: 读取 {SETTINGS_FILE} 时核心程序部分发生错误：{e}。")
        else:
    ...
    def save_core_program_path_setting(self):
        ...
        if os.path.exists(self.settings_file_path):
            try:
                ...
                print(f"save_core_program_path_setting: 读取现有设置文件时发生错误，将创建新文件：{e}")
    ...
    def _prompt_for_core_program_file(self):
        """
        打开文件对话框让用户选择核心程序可执行文件 (idv-login*.exe)。
        返回选定的文件路径或 None。
        """
        ...
        initial_dir = os.path.dirname(self.core_program_path) if self.core_program_path and os.path.exists(
            os.path.dirname(self.core_program_path)) else self.current_script_dir
        ...
        if file_dialog.exec_():
            ...
            if selected_files:
                ...
                # 简单检查文件名是否包含 'idv-login'，如果不是则提示
                if "idv-login" not in os.path.basename(selected_path).lower():
                    reply = QMessageBox.question(self, "确认核心程序",
                                                 f"您选择的文件 '{os.path.basename(selected_path)}' 的名称不包含 'idv-login'。您确定这是正确的核心程序吗？",
                                                 QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                    if reply == QMessageBox.No:
                        print("prompt_for_core_program_file: 用户因文件名不匹配而取消了文件选择。")
                        return None
    ...
    def start_core_program(self):
        """
        在启动器启动时，查找并启动以“idv-login”开头的核心程序，
        并默认隐藏其控制台窗口。
        """
        ...
        if self.core_program_path and os.path.exists(self.core_program_path) and os.path.isfile(self.core_program_path):
            ...
            print(f"start_core_program: 从设置加载的核心程序路径 {core_program_to_launch} 被验证为有效。")
        else: # This 'else' block is the key.
            print(f"start_core_program: 从设置加载的核心程序路径 ({self.core_program_path}) 无效或不存在，尝试其他查找方法。")
            # 尝试2: 在当前执行目录中搜索 idv-login*.exe
            print("start_core_program: 在当前目录搜索 'idv-login*.exe'。")
            exe_files = glob.glob(os.path.join(self.current_script_dir, "idv-login*.exe"))
            if exe_files:
                ...
                print(f"start_core_program: 在当前目录找到核心程序: {core_program_to_launch}，并已保存。")
            else:
                # 尝试3: 如果仍然未找到，提示用户手动选择
                print("start_core_program: 自动搜索未找到核心程序，提示用户手动选择。")
                ...
                if selected_path:
                    ...
                    QMessageBox.warning(self, "核心程序未找到", "未能找到核心程序。核心功能可能无法使用。")
                    print("start_core_program: 用户取消选择核心程序，核心程序功能可能无法使用。")
        ...
        try:
            ...
            # 使用 ShellExecuteW 以管理员权限启动核心程序
            # 这里的 "runas" 是关键，它会触发 UAC 提示以请求管理员权限
            ...
            print(f"start_core_program: ShellExecuteW 启动核心程序返回值: {ret_val}")

            if ret_val <= 32:  # 返回值 <= 32 表示错误
                error_code = ctypes.windll.kernel32.GetLastError()
                QMessageBox.critical(self, "核心程序启动错误",
                                     f"启动核心程序失败 (ShellExecuteW 失败)。错误代码: {error_code}\n路径: {core_program_to_launch}\n\n请确保该程序可以以管理员身份运行。")
                print(f"start_core_program: ShellExecuteW 启动核心程序失败，错误代码: {error_code}")
                self.core_program_process = None  # 无法启动进程，清空引用
                return

            print(f"start_core_program: 尝试以管理员身份启动核心程序：{core_program_to_launch}")

            # ShellExecuteW 启动的进程没有直接的 Popen 对象，所以我们无法直接跟踪其PID。
            # 为了能够控制其窗口，我们需要在一段时间后查找其窗口句柄。
            # 这里我们不设置 self.core_program_process，因为它不再是 Popen 对象。
            # 但是，我们可以模拟一个简单的 QProcess 来处理窗口句柄查找。
            # 为了简化，我们只依赖于 QTimer 和 _find_window_handle_by_exe_name。
            # 我们假设如果 ShellExecuteW 成功返回，进程就开始运行了。

            # 使用 QTimer 异步查找窗口句柄，用于后续的显示/隐藏切换
            ...
            QMessageBox.critical(self, "核心程序启动错误",
                                 f"启动核心程序失败。\n错误：{e}\n\n请确认 '{os.path.basename(core_program_to_launch)}' 是否存在且为有效可执行文件。如果它是命令行程序，它应该创建自己的控制台；如果它是 GUI 程序，此错误可能表明它无法通过此方式启动。")
    ...
    def _get_core_program_console_handle(self, exe_name_to_find):  # 确保这里接受 exe_name_to_find
        ...
        # 这里不再检查 self.core_program_process.poll()，因为 ShellExecuteW 没有 Popen 对象。
        ...
        print(
            f"_get_core_program_console_handle: 尝试获取核心程序（名称以 '{exe_name_to_find}' 开头）的控制台窗口句柄...")
        ...
        if self.core_program_window_handles:
            ...
            if user32.IsWindowVisible(self.core_program_window_handles[0]):
                ...
                print("_get_core_program_console_handle: 警告：至少一个核心程序窗口未按预期在启动时隐藏。")
            else:
                self.core_program_hidden = True
        else:
            print(
                "_get_core_program_console_handle: 警告：未能找到任何核心程序控制台窗口句柄。这可能意味着它不是控制台应用程序，或者窗口尚未创建。控制台显示/隐藏功能将无法工作。")
            self.core_program_hidden = False  # 如果未找到句柄，重置为 False，因为我们无法控制它。

    def load_game_path_setting(self):
        """
        从配置文件中加载游戏可执行文件路径。
        如果文件不存在或读取失败，则返回默认路径。
        """
        print("load_game_path_setting: 尝试加载设置。")
        default_path = "D:/dwrg2/dwrg.exe"  # 您的默认游戏路径
        if os.path.exists(self.settings_file_path):
            try:
                with open(self.settings_file_path, 'r', encoding='utf-8') as f:
                    settings = json.load(f)
                ...
                print(f"load_game_path_setting: 错误：解析 {SETTINGS_FILE} 时发生 JSON 解码错误：{e}。使用默认路径。")
            except Exception as e:
                print(f"load_game_path_setting: 读取 {SETTINGS_FILE} 时发生错误：{e}。使用默认路径。")
        else:
    ...
    def save_game_path_setting(self):
        ...
        if os.path.exists(self.settings_file_path):
            try:
                ...
                print(f"save_game_path_setting: 读取现有设置文件时发生错误，将创建新文件：{e}")
    ...
    def _terminate_process(self, process, name):
        ...
        if isinstance(process, QProcess):
            ...
            if process.poll() is None:  # 检查进程是否仍在运行
                ...
                try:
                    process.wait(timeout=1)  # 等待一小段时间进行终止
                except subprocess.TimeoutExpired:
                    process.kill()  # 如果未终止，则强制杀死
    ...
    def start_background_scripts(self):
        ...
        if os.path.exists(htmlget_script_path):
            ...
            # 添加一个标志，指示它是否在打包的应用程序中运行，如果 htmlget.py 需要的话
    ...
    def handle_htmlget_finished(self, exitCode, exitStatus):
        ...
        print(f"handle_htmlget_finished: htmlget.py 进程完成，退出代码: {exitCode}，状态: {exitStatus}")
        if exitCode == 0:
            # 如果 htmlget.py 成功完成，立即尝试加载数据
            self.check_and_reload_data()
            # 如果数据文件尚未完全准备好（例如，文件系统延迟），启动计时器继续检查
            ...
            if not (os.path.exists(data_file_path) and os.path.exists(bg_img_path) and os.path.exists(
                ...
                if not self.data_check_timer.isActive():
                    ...
                    print("handle_htmlget_finished: htmlget.py 成功完成，启动数据文件检查计时器。")
            else:
                print("handle_htmlget_finished: htmlget.py 成功完成，所有预期数据文件已就绪，无需启动计时器。")

        else:
            QMessageBox.warning(self, "数据抓取失败", "新闻和背景数据抓取脚本失败。请检查命令行输出或日志文件。")
            # 如果进程失败，如果计时器正在运行，则停止计时器，因为不再期望数据成功生成
    ...
    def load_scraped_data_json(self):
        ...
        try:
            with open(data_file_path, 'r', encoding='utf-8') as f:
                ...
                data["season"] = json_data.get("season", "赛季信息加载中...")
            ...
            print(f"load_scraped_data_json: 错误：解析 web_data.json 时发生 JSON 解码错误：{e}，使用默认值。")
        except Exception as e:
            print(f"load_scraped_data_json: 读取或解析 web_data.json 时发生错误：{e}，使用默认值。")
        return data

    def check_and_reload_data(self):
        """
        检查数据文件和图片是否已生成，如果已生成且数据有变化，则重新加载数据并更新 UI。
        """
        print("check_and_reload_data: 检查数据文件和图片。")
        data_file_path = os.path.join(self.res_dir, "web_data.json")
        bg_img_path = os.path.join(self.res_dir, "bg_img.jpg")
        # 检查至少第一张新闻图片是否存在作为数据完成的标志
        first_news_img_path = os.path.join(self.res_dir, "new1_img.jpg")

        # 仅当数据文件和必要的图片存在时才尝试重新加载和更新 UI
        if os.path.exists(data_file_path) and os.path.exists(bg_img_path) and os.path.exists(first_news_img_path):
            ...
            if new_scraped_data != self.scraped_data:
                ...
                self.populateNews()
                self.start_news_carousel()  # 新数据到来时，重新启动轮播
                self.applyStyles()  # 重新应用样式以更新背景图片
                print("check_and_reload_data: JSON 数据文件成功加载并更新 UI。")
                # 数据已加载且 UI 已更新，停止计时器
                if self.data_check_timer.isActive():
                    self.data_check_timer.stop()
                    print("check_and_reload_data: 数据已加载，数据检查计时器已停止。")
            else:
                # 数据未改变，但如果计时器正在运行且数据已完成，则停止计时器
                if self.data_check_timer.isActive() and \
                        self.scraped_data["season"] != "赛季信息加载中..." and \
                        self.scraped_data["news_list"]:
                    self.data_check_timer.stop()
                    print("check_and_reload_data: 数据未改变但已完成，数据检查计时器已停止。")
        else:
            print("check_and_reload_data: 数据文件或图片仍在等待生成...")
    ...
    def initUI(self):
        ...
        self.version_status_label = QLabel("赛季信息加载中...")
        ...
        self.small_preview_label.setText("活动预览图")
        ...
        self.activities_and_news_list.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # 关闭水平滚动条
        self.activities_and_news_list.setStyleSheet("""
            ...
            QListWidget QLabel {{ /* 列表项内的 QLabel 样式 */
                color: #cccccc;
            }}
            QListWidget QLabel#time_label {{ /* 列表项内时间标签的样式 */
                color: #999999;
            }}
        """)
        ...
        right_play_area_layout.addStretch(40)  # 顶部弹簧用于整体垂直定位
        ...
        # --- “定位游戏”文本框的垂直位置微调变量 ---
        # 调整此值以控制“定位游戏”文本框与上方按钮的距离。
        # 增加值会使其向下移动，减小值会使其向上移动。
        ...
        self.locate_game_container.setCursor(Qt.PointingHandCursor)  # 鼠标悬停时显示手型光标
        ...
        self.locate_game_label = QLabel("已安装？定位游戏")
        self.locate_game_label.setAlignment(Qt.AlignRight | Qt.AlignVCenter)  # 文本右对齐并垂直居中
        self.locate_game_label.setFixedSize(250, 25)  # 标签内部大小保持不变
        ...
        locate_game_outer_layout.addSpacing(520)  # 在左侧添加 520px 的固定空间以将其向右推
        locate_game_outer_layout.addWidget(self.locate_game_container)
        locate_game_outer_layout.addStretch()  # 添加一个弹簧以将剩余空间推向右侧，使文本框左对齐

        right_play_area_layout.addLayout(locate_game_outer_layout)
        right_play_area_layout.addStretch(5)  # 在底部添加一个弹簧，以确保上方元素尽可能高
    ...
    def open_settings_placeholder(self):
        ...
        在其他平台或核心程序未运行时，显示通用消息。
        """
        print("open_settings_placeholder: 设置按钮被点击。")
        # 检查 core_program_process 是否存在并且其引用不是 None，并且至少找到了一个窗口句柄
        # 否则，如果核心程序是由 ShellExecuteW 启动的，self.core_program_process 将为 None
        # 我们应该直接使用 self.core_program_window_handles 来判断是否可以控制窗口
    ...
    def update_game_path_ui(self):
        ...
        if game_installed:
            ...
            if not self.game_check_timer.isActive() and (
                ...
                self.start_game_btn.setEnabled(False)  # 游戏正在启动或运行中，禁用按钮
                print("update_game_path_ui: 游戏已安装，但游戏检查计时器正在运行或进程正在运行，按钮已禁用。")

            self.btn_text_label.setText("开始游戏")
            ...
            # 应用“开始游戏”按钮的 QSS 样式 (移除了不支持的 box-shadow 和 transition)
            ...
            print("update_game_path_ui: 定位游戏容器已隐藏。")

        else:  # 游戏未安装
            self.start_game_btn.setEnabled(True)  # 未安装时，总是可以点击“获取游戏”
            ...
            # 应用“获取游戏”按钮的 QSS 样式 (移除了不支持的 box-shadow 和 transition)
            self.start_game_btn.setStyleSheet(f"""
                QPushButton {{
            ...
            print("update_game_path_ui: 定位游戏容器已显示。")
    ...
    def eventFilter(self, obj, event):
        """
        事件过滤器，用于处理按钮的悬停事件，切换图标和文本颜色。
        """
        # --- 处理按钮悬停事件（仅颜色和阴影变化，无移动） ---
        if obj == self.start_game_btn:
            if event.type() == QEvent.Enter:
                # 鼠标进入按钮区域
                if self.btn_text_label.text() == "开始游戏":
                    # 切换到悬停图标和文本颜色
                    play_icon_path_green = os.path.join(self.res_dir, "play_icon_green.png")
    ...
    def locate_game_file(self, event):
        """
        处理“定位游戏”的点击事件，打开文件对话框让用户选择游戏可执行文件。
        """
        # 确保事件被接受以防止传播
        event.accept()

        print("locate_game_file: 用户点击了定位游戏。")
        file_dialog = QFileDialog(self)
        file_dialog.setWindowTitle("定位游戏可执行文件")
        file_dialog.setFileMode(QFileDialog.ExistingFile)
            ...
            if selected_files:
                ...
                if os.path.basename(new_path).lower() != "dwrg.exe":
                    reply = QMessageBox.question(self, "确认游戏文件",
                                                 ...
                                                 QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                    if reply == QMessageBox.No:
                        print("locate_game_file: 用户因文件名不正确而取消了文件选择。")
                        return

                self.game_exe_path = new_path
                self.save_game_path_setting()  # 保存新的游戏路径
    ...
    def _check_running_game_status(self):
        """
        Windows 专用：定期检查游戏是否已启动并创建了其主窗口。
        """
        print(
        ...
        if game_window_handles:
            print("_check_running_game_status: 检测到游戏窗口，游戏已成功启动。")
            self.game_check_timer.stop()
            self.game_check_attempts = 0
            self.game_process = None  # 我们没有 ShellExecuteW 的 Popen 对象，此处重置为 None
            self.update_game_path_ui()  # 重新启用按钮
        elif self.game_check_attempts >= self.max_game_check_attempts:
            print("_check_running_game_status: 达到最大检查次数，游戏窗口未被检测到。假设启动失败。")
            self.game_check_timer.stop()
            self.game_check_attempts = 0
            self.update_game_path_ui()  # 重新启用按钮
            QMessageBox.warning(self, "游戏启动警告", "未能检测到游戏窗口。游戏可能未正常启动或已崩溃。")

    def start_game(self):
        """
        启动游戏。尝试以管理员权限运行。
        如果当前按钮显示“获取游戏”，则打开下载链接。
        """
        ...
        self.btn_text_label.setText("游戏启动中...")  # 更新按钮文本
        ...
        try:
            if sys.platform == "win32":
                ...
                print(f"start_game: ShellExecuteW 返回值: {ret_val}")
                if ret_val <= 32:  # 返回值 <= 32 表示错误
                    error_code = ctypes.windll.kernel32.GetLastError()
                    QMessageBox.critical(self, "启动错误",
                                         f"启动游戏失败 (ShellExecuteW 失败)。错误代码: {error_code}\n路径: {self.game_exe_path}")
                ...
                self._terminate_process(self.game_process, "Game")
    ...
    def updateDynamicContent(self):
        ...
        更新动态内容，如赛季信息标签。
        """
        print("updateDynamicContent: 更新动态内容（赛季信息）。")
        ...
        if not self.scraped_data["news_list"]:
            print("updateDynamicContent: 没有新闻数据，活动预览图片使用默认占位符。")
            self.small_preview_label.setText("活动预览图")
    ...
    def populateNews(self):
        ...
        if self.scraped_data["news_list"]:
            for news_item in self.scraped_data["news_list"]:
                title = news_item.get("title", "无标题新闻")
                ...
                if url:
                    item.setData(Qt.UserRole, url)  # 将 URL 存储在用户角色数据中
                else:
                    item.setData(Qt.UserRole, "")
        else:
            item = QListWidgetItem("暂无新闻。请等待数据加载或检查脚本执行。")
    ...
    def _set_preview_image(self, index):
        ...
        if not self.scraped_data["news_list"] or not (0 <= index < len(self.scraped_data["news_list"])):
            self.small_preview_label.setText("无图片或索引无效")
            return
        ...
        if image_path and os.path.exists(image_path):
            ...
            if not pixmap.isNull():
                ...
                self.small_preview_label.setText("图片加载失败")
        else:
            self.small_preview_label.setText("图片缺失")
    ...
    def start_news_carousel(self):
        ...
        if self.scraped_data["news_list"]:
            ...
            self.current_news_index = 0  # 启动或重新启动时从第一个新闻项开始

            self._set_preview_image(self.current_news_index)
            if self.news_carousel_timer.isActive():
                self.news_carousel_timer.stop()  # 如果正在运行，先停止
    ...
    def handle_swipe(self, direction):
        """
        处理 SwipeableLabel 的滑动事件，手动切换新闻轮播图片。
        """
        if not self.scraped_data["news_list"]:
            return
    ...
    def openNewsLink(self, item):
        ...
        if news_url:
            ...
            print(f"openNewsLink: 正在打开链接: {news_url}")
        else:
    ...
    def _set_background_image(self):
        ...
        设置主窗口的背景图片。
        """
        print("_set_background_image: 设置主窗口背景图片。")
        ...
        if background_image_path and os.path.exists(background_image_path):
            ...
            if not pixmap.isNull():
                ...
                print(f"_set_background_image: 背景图片已成功从本地加载并应用：{background_image_path}")
            else:
                print(f"_set_background_image: 错误：未能从 {background_image_path} 加载背景图片。")
        else:
            print(f"_set_background_image: 错误：未找到背景图片文件或路径无效：{background_image_path}，使用默认纯色背景。")
            palette = self.palette()
            palette.setColor(QPalette.Background, QColor("#1a1a2a"))  # 设置默认背景色
            self.setPalette(palette)

    def applyStyles(self):
        """
        应用全局和特定小部件的样式，包括字体和背景。
        """
    ...
    def resizeEvent(self, event):
        """
        处理窗口大小调整事件，重新设置背景和预览图片。
        """
        super().resizeEvent(event)
        print("resizeEvent: 窗口已调整大小，重新设置背景和预览图片。")
        self._set_background_image()
        self._set_preview_image(self.current_news_index)
    ...
    def closeEvent(self, event):
        ...
        处理窗口关闭事件，终止所有后台进程。
        """
        print("closeEvent: 主应用程序正在关闭，终止后台进程...")
        self._terminate_process(self.htmlget_process, "htmlget.py")
        # 确保游戏进程检查计时器已停止
        if self.game_check_timer.isActive():
            self.game_check_timer.stop()
        self._terminate_process(self.game_process, "Game")  # 终止实际的游戏进程（如果存在）
        # 如果核心程序进程由启动器启动且仍在运行，则终止它
        # 对于 ShellExecuteW 启动的进程，我们没有 Popen 对象来终止。
        # 只能依赖于操作系统在主程序退出时清理。
        # 这里只清理由 Popen 或 QProcess 明确跟踪的进程。
        if isinstance(self.core_program_process, subprocess.Popen) and self.core_program_process.poll() is None:
            self._terminate_process(self.core_program_process, "Core Program (Popen)")
...
def acquire_single_instance_lock():
    """
    尝试获取应用程序的单例锁。
    如果成功获取锁，则返回 False（表示这是第一个实例）。
    如果无法获取锁（因为另一个实例已运行），则返回 True。
    此功能仅适用于 Windows 平台。
    """
    ...
    try:
        ...
        msvcrt.locking(_app_lock_file.fileno(), msvcrt.LK_NBLCK, 1)  # LK_NBLCK (non-blocking)
        print(f"acquire_single_instance_lock: 成功获取单例锁: {lock_file_path}")
        return False  # 成功获取锁，这是第一个实例
    except IOError:
        print(f"acquire_single_instance_lock: 无法获取单例锁，另一个实例可能已运行: {lock_file_path}")
        return True  # 无法获取锁，另一个实例已运行
    except Exception as e:
        print(f"acquire_single_instance_lock: 获取单例锁时发生错误: {e}")
        return True  # 发生错误，也视为已运行，避免多开
...
def release_single_instance_lock():
    ...
    if _app_lock_file:
        try:
            msvcrt.locking(_app_lock_file.fileno(), msvcrt.LK_UNLCK, 1)  # LK_UNLCK (unlock)
            _app_lock_file.close()
            print("release_single_instance_lock: 已释放单例锁。")
        except Exception as e:
            print(f"release_single_instance_lock: 释放单例锁时发生错误: {e}")
        _app_lock_file = None
...
def excepthook(exc_type, exc_value, exc_traceback):
    ...
    print(f"Unhandled exception caught by excepthook:\n{error_message}")
    # 可以将错误写入日志文件，或者显示一个 QMessageBox
    QMessageBox.critical(None, "应用程序错误", f"发生了一个未处理的错误：\n{error_message}\n\n应用程序即将退出。")
    # 确保在退出前释放锁，如果应用程序在初始化阶段崩溃
...
if __name__ == '__main__':
    ...
    # 解决 PyInstaller 打包时 multiprocessing 模块在 Windows 上创建子进程的问题
    # 必须放在 if __name__ == '__main__': 的下一行，确保在所有进程入口点都能调用
    multiprocessing.freeze_support()
    print("__main__: 启动 QApplication。")

    # 尝试设置高 DPI 缩放，以便在高分辨率屏幕上获得更清晰的 UI
    # 注意：这些属性必须在 QCoreApplication（QApplication的基类）实例创建之前设置。
    try:
        if hasattr(Qt, 'AA_EnableHighDpiScaling'):
            QApplication.setAttribute(Qt.AA_EnableHighDpiScaling)
            print("__main__: 已启用高 DPI 缩放。")
        if hasattr(Qt, 'AA_UseHighDpiPixmaps'):
            QApplication.setAttribute(Qt.AA_UseHighDpiPixmaps)
            print("__main__: 已启用高 DPI 像素图。")
        # 对于 Windows 10+，确保应用程序 DPI 感知
        myappid = 'mycompany.myproduct.gamelaucher.1_0'  # 任意字符串
        if sys.platform == "win32":  # 仅在 Windows 上应用
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(myappid)
            print(f"__main__: 已将 AppUserModelID 设置为 {myappid}。")
    except AttributeError as e:
        print(f"__main__: DPI 设置属性错误: {e}。跳过 DPI 设置。")
    except Exception as e:
        print(f"__main__: DPI 设置或 AppUserModelID 时发生一般错误: {e}")

    app = QApplication(sys.argv)  # QApplication 实例现在在 DPI 设置之后创建

    print("__main__: 检查单例。")
    # --- 检查是否已有实例运行 ---
    if acquire_single_instance_lock():
        # 如果已经有一个实例在运行，则显示消息并退出。
        # 由于 QApplication 已经初始化，我们可以直接使用 QMessageBox。
        QMessageBox.warning(None, "启动器已运行", "游戏启动器已在运行中。请检查任务栏或系统托盘。")
        print("__main__: 另一个实例正在运行。正在退出。")
        sys.exit(0)  # 正常退出
